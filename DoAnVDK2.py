...
def GUI():
    app = App(title="Đồ Án Vi Điều Khiển Khanh + Toàn", layout="grid", width=478, height=120)
    result = [None]
    face_result = [False]
    def open_password_window():
        nhap_pass = Window(app, title="Nhập Mật Khẩu Số", layout="grid", width=190, height=260)
        textbox = TextBox(nhap_pass, grid=[0, 0, 3, 1], width=10)
        ...
        def set_number(number):
            ...
            if len(current_text) < 4:
                ...
                if len(textbox.value) == 4:
                    ...
                    nhap_pass.after(100, lambda: [nhap_pass.destroy(), app.destroy()])
        def on_closing():
            result[0] = None
            nhap_pass.destroy()
        PushButton(nhap_pass, text="1", command=set_number, args=["1"], grid=[0, 1], width=5, height=2)
        PushButton(nhap_pass, text="2", command=set_number, args=["2"], grid=[1, 1], width=5, height=2)
        PushButton(nhap_pass, text="3", command=set_number, args=["3"], grid=[2, 1], width=5, height=2)
        PushButton(nhap_pass, text="4", command=set_number, args=["4"], grid=[0, 2], width=5, height=2)
        PushButton(nhap_pass, text="5", command=set_number, args=["5"], grid=[1, 2], width=5, height=2)
        PushButton(nhap_pass, text="6", command=set_number, args=["6"], grid=[2, 2], width=5, height=2)
        PushButton(nhap_pass, text="7", command=set_number, args=["7"], grid=[0, 3], width=5, height=2)
        PushButton(nhap_pass, text="8", command=set_number, args=["8"], grid=[1, 3], width=5, height=2)
        PushButton(nhap_pass, text="9", command=set_number, args=["9"], grid=[2, 3], width=5, height=2)
        PushButton(nhap_pass, text="0", command=set_number, args=["0"], grid=[0, 4, 3, 1], width=23, height=2, align="center")
        nhap_pass.tk.protocol("WM_DELETE_WINDOW", on_closing)
    def face_id_action():
        print("Face_ID được chọn")
        face_result[0] = NDKM()
        app.destroy()
    PushButton(app, text="PASSWORD", command=open_password_window, grid=[0, 1], width=30, height=6)
    PushButton(app, text="FACE_ID", command=face_id_action, grid=[1, 1], width=30, height=6)
    app.display()
    if face_result[0]:
        ...
        if program_stt == 0:
            ...
            while time.time() - start_time < 40:
                ...
                if fromMSP430 == 'C':
                    ...
                    new_pw = input_password(send_to_msp430=False)
                    if new_pw:
                        doi_mk(new_pw)
                    ser.write(b'D')
            else:
